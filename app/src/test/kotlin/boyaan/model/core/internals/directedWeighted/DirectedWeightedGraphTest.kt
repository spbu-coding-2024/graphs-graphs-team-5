...
internal class DirectedWeightedGraphTest {
    @Nested
    ...
    inner class EdgeTest {
        ...
        inner class AddTest {
            ...
            fun `addEdge adds edge only for one direction`() {
                val ac = graph.addEdge(0, 2, "ac", 4.5)
                assertEquals(1, graph.edges.size)
                assertNotNull(ac)
                assertNull(graph[2, 0])
                val ca = graph.addEdge(2, 0, "ca", 3.56)
                assertEquals(2, graph.edges.size)
                assertNotNull(ca)
                assertEquals(4.5, ac?.weight)
            }
        ...
        inner class GetTest {
            ...
            fun `can get edges only via valid keys`() {
                ...
                assertEquals(0.5, graph[1, 3]?.weight)
        ...
        inner class SetValueTest {
            ...
            fun `can change any edge value`() {
                ...
                graph[2, 0]?.value = "ccaa"

                assertEquals("bbcc", graph[0, 2]?.value)
                assertEquals("aacc", graph[1, 3]?.value)
                assertEquals("bbdd", graph[1, 2]?.value)
                assertEquals("ccaa", graph[2, 0]?.value)
            }

            @Test
            fun `can change any edge weight`() {
                graph[0, 2]?.weight = 12.2
                graph[1, 3]?.weight = 8.8
                graph[1, 2]?.weight = 10.0
                graph[2, 0]?.weight = 12.125

                assertEquals(12.2, graph[0, 2]?.weight)
                assertEquals(8.8, graph[1, 3]?.weight)
                assertEquals(10.0, graph[1, 2]?.weight)
                assertEquals(12.125, graph[2, 0]?.weight)
            }
        }
